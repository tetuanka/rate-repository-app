  ...
  describe('RepositoryListContainer', () => {
    it('renders repository information correctly', () => {
      ...
      repositoryItems.forEach((repositoryItem, index) => {
        ...
        expect(repositoryItem).toHaveTextContent(`${formatNumber(repository.stargazersCount)}`);
        expect(repositoryItem).toHaveTextContent(`${formatNumber(repository.forksCount)}`);
        expect(repositoryItem).toHaveTextContent(`${formatNumber(repository.reviewCount)}`);
        expect(repositoryItem).toHaveTextContent(`${formatNumber(repository.ratingAverage)}`);
      });
    });
  });
});
